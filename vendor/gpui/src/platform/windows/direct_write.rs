...
impl DirectWriteTextSystem {
    pub(crate) fn new(directx_devices: &DirectXDevices) -> Result<Self> {
        ...
        // HARMONIYA PATCH: build the (currently empty) custom collection first so
        // it can double as a fallback for the system collection below.
        ...
        // HARMONIYA PATCH: some Windows installs have a corrupt font cache or bad
        // HKLM font-registry entries, so GetSystemFontCollection fails with
        // ERROR_MORE_DATA (0x800700EA) and aborts startup. We only ever render
        // bundled fonts, so the system collection is just fallback/enumeration we
        // never exercise — on failure, degrade to the empty custom collection
        // instead of crashing the whole app. (Upstream gpui 0.2.2 had a bare `?`
        // here; see vendor/gpui/HARMONIYA_PATCH.md.)
        let system_font_collection = unsafe {
                ...
                .GetSystemFontCollection(false, &mut result, true)
            {
                ...
                Err(err) => {
                    log::error!(
                        "GetSystemFontCollection failed ({err:?}); falling back to an \
                         empty font collection — system fonts will be unavailable"
                    );
                    custom_font_collection.clone()
                }
            }
        };
