...
pub(crate) fn updating_view(phase: &UpdatePhase) -> AnyElement {
    ...
    rsx! {
        <div size_full flex flex_col items_center justify_center gap={px(22.)}>
            <div flex flex_col items_center gap={px(8.)}>
                <div text_size={px(22.)} font_weight={FontWeight::BOLD} text_color={Theme::text()}>
                    {title}
                </div>
                <div text_size={px(13.)} text_color={Theme::text_muted()}>{subtitle}</div>
            </div>
            {indeterminate_bar()}
        </div>
    }
...
fn indeterminate_bar() -> AnyElement {
    rsx! {
        <div relative w={px(BAR_W)} h={px(4.)} rounded_full bg={Theme::surface_raised()} overflow_hidden>
            <div
                absolute
                top={px(0.)}
                h={px(4.)}
