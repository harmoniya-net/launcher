...
fn btn_fill(fill: impl Into<Hsla>) -> Div {
    rsx! {
        <div absolute inset_0 size_full rounded={px(BTN_RADIUS)} bg={fill.into()} />
    }
}
...
pub(crate) fn play_button(
    ...
    if running {
        ...
        rsx! {
            ...
            >
                {btn_fill(rgb(STOP_BG))}
                {btn_content(Some("icons/power.svg"), crate::i18n::t().stop, Theme::text())}
            </div>
        }
        .into_any_element()
        ...
        let mut btn = rsx! {
            <div id="play" relative flex_shrink_0 w={px(BTN_W)} h={px(BTN_H)} cursor_pointer on_hover={on_hover}>
                {btn_fill(rgb(0xffffff))}
                {btn_content(Some("icons/play.svg"), label, Theme::on_accent())}
            </div>
        };
        if play_hovered {
            ...
            let green = rsx! {
                <div absolute left={px(0.)} bottom={px(0.)} w={px(BTN_W)} overflow_hidden flex flex_col justify_end>
                    <div flex_shrink_0 relative w={px(BTN_W)} h={px(BTN_H)}>
                        {btn_fill(rgb(HOVER_GREEN))}
                        {btn_content(Some("icons/play.svg"), label, Theme::text())}
                    </div>
                </div>
            }
            .with_animation(
        ...
        rsx! {
            <div relative flex_shrink_0 w={px(BTN_W)} h={px(BTN_H)}>
                {btn_fill(Theme::surface())}
                {btn_content(None, label, color)}
            </div>
        }
        .into_any_element()
    }
